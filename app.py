...
def limpiar_pantalla_total():
    ...
    for key in list(st.session_state.keys()):
        if key.startswith(('c_', 'e_', 'p_', 'o_', 'd_', 'pr_', 'curva_', 'add_')):
        ...
        try:
            ...
            if seleccion != "Seleccionar...":
                ...
                if st.button("Abrir Ficha"):
                    ...
                    st.session_state.bloquear = True
                    st.session_state.confirmar_envio = False
                    st.session_state.insumos_temp = [] # Se limpia para cargar los de la DB
                    st.rerun()
    ...
    with tab1:
        ...
        if not es_nuevo:
            ...
            if res.data:
                ...
                # Cargar insumos de DB a la sesión si está vacía
        ...
        with st.container(border=True):
            ...
            with cols_t[0]: st.write("**Corte**")
            db_curva = datos_db.get('curva_tallas', {}) if isinstance(datos_db.get('curva_tallas'), dict) else {}
            for i, t in enumerate(tallas_lista):
                with cols_t[i+1]:
                    curva_datos[t] = st.number_input(f"T{t}", min_value=0, value=int(db_curva.get(t, 0)), label_visibility="collapsed", key=f"curva_{t}", disabled=st.session_state.bloquear or ya_enviado)
            
            st.divider()
            cc1, cc2 = st.columns([1, 2])
            with cc1:
                val_paq = st.number_input("Paquetes", min_value=1, value=int(datos_db.get('cantidad_paquetes', 1)), disabled=st.session_state.bloquear or ya_enviado)
            with cc2:
                total_general = sum(curva_datos.values()) * val_paq
                st.metric("TOTAL PRENDAS", total_general)
